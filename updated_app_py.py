...
class ExcelTransformer:
    ...
    def transform_to_metadata(self, df, report_type):
        ...
        for idx, column in enumerate(df.columns, 1):
            ...
            # Генерируем код атрибута
            attribute_code = f"{self.report_number}_{idx:03d}"
            ...
            metadata_record = {
                'ReportCode_info': self.report_number,
    ...
    def create_excel_download(self, metadata_df):
        """
        Создание Excel файла для скачивания с заголовками и скрытой строкой
        ...
        for row_idx, (_, row) in enumerate(metadata_df.iterrows(), 2):
        ...
        try:
            ...
            if st.button("🔄 Выгрузить атрибутный состав", type="primary", use_container_width=True):
                with st.spinner("Преобразование данных..."):
                    ...
                    st.info("💡 **Совет**: Файл содержит скрытую строку с техническими заголовками для системной обработки.")
